 ...
 * −
 * 
 * Sequence of elements − A stream provides a set of elements of specific type
...
public class Stream {
	...
	public void test_Stream() {
		...
		/**
		 * Collectors Collectors are used to combine the result of processing on the
		 * elements of a stream. Collectors can be used to return a list or a string.
		 */
		...
		/**
		 * Statistics With Java 8, statistics collectors are introduced to calculate all
		...
		String[] receipt = {"", "sfds",};
		Arrays.stream(receipt).filter(email -> email != null && email.length() > 2).toArray(size -> new String[size]);
